...
class Login extends React.Component {
    ...
    onFormSubmit = async (e) => {
        ...
        if (this.validationState('email') === 'success' && this.validationState('password') === 'success') {
            ...
            if (result.status === 200 && result.data.success.token 
            && result.data.success.role && result.data.success.name) {
                this.props.saveToken(result.data.success.token, result.data.success.userid, 
                                     result.data.success.role, result.data.success.name);
                if (parseInt(result.data.success.role) === 1) {
                    this.props.history.push('/users');
                }
                else if (parseInt(result.data.success.role) === 2) {
